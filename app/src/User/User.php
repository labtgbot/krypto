...
class User extends MySQL {
  ...
  const RESET_PASSWORD_TOKEN_TTL_SECONDS = 3600;
  ...
  private $resetTokensNeedInvalidation = false;
  ...
  /**
   * Generate a password reset token value with CSPRNG entropy.
   * @return String Reset token
   */
  public static function _generateResetPasswordTokenValue(){
    return bin2hex(random_bytes(32));
  }

  public static function _generateOauthAccountSecret(){
    return 'oauth:'.bin2hex(random_bytes(32));
  }

  private static function _storedCredentialForNewUser($password, $oauth, $setpwd){
    if($oauth == 'standard') return self::_hashPassword($password);
    if($setpwd) return $password;
    return self::_generateOauthAccountSecret();
  }

  /**
   * Validate the format of password reset tokens generated by this version.
   * @param  String  $token Reset token
   * @return Boolean
   */
  public static function _isValidResetPasswordToken($token){
    return is_string($token) && preg_match('/\A[0-9a-f]{64}\z/', $token) === 1;
  }

  /**
   * Check if a password reset token creation timestamp has expired.
   * @param  String|Int $createdAt Token creation timestamp
   * @param  Int|null   $now       Current timestamp, injectable for tests
   * @return Boolean
   */
  public static function _isResetPasswordTokenExpired($createdAt, $now = null){
    if(is_null($now)) $now = time();
    if(is_null($createdAt) || $createdAt === '') return true;

    $createdAt = (string) $createdAt;
    if(!ctype_digit($createdAt)) return true;

    $createdAt = (int) $createdAt;
    if($createdAt <= 0) return true;

    return ((int) $now - $createdAt) > self::RESET_PASSWORD_TOKEN_TTL_SECONDS;
  }
  ...
  public function _oauthCallback($oauth){
    ...
    if(method_exists($oauth, '_getId')) return $this->_oauthCallbackID($oauth);
    throw new Exception("Error : OAuth provider subject id required", 1);

  }

  private function _migrateLegacyOauthIdentity($oauth, $oauthId){

    if(!method_exists($oauth, '_getEmail')) return null;

    $email = (string) $oauth->_getEmail();
    $oauthName = (string) $oauth->_getOauthName();
    if(!filter_var($email, FILTER_VALIDATE_EMAIL)) return null;

    $legacyRows = parent::querySqlRequest("SELECT * FROM user_krypto WHERE email_user=:email_user AND oauth_user=:oauth_user AND password_user=:legacy_password",
                                          [
                                            'email_user' => $email,
                                            'oauth_user' => $oauthName,
                                            'legacy_password' => $oauthName
                                          ]);
    if(count($legacyRows) == 0) return null;

    $updated = parent::execSqlRequest("UPDATE user_krypto SET password_user=:oauth_id WHERE id_user=:id_user AND oauth_user=:oauth_user AND password_user=:legacy_password",
                                      [
                                        'oauth_id' => $oauthId,
                                        'id_user' => $legacyRows[0]['id_user'],
                                        'oauth_user' => $oauthName,
                                        'legacy_password' => $oauthName
                                      ]);
    if(!$updated) throw new Exception("Error : Fail to migrate OAuth identity", 1);

    $legacyRows[0]['password_user'] = $oauthId;
    return $legacyRows[0];
  ...
  public function _oauthCallbackID($oauth){
    ...
    $oauthId = (string) $oauth->_getId();
    if(trim($oauthId) === '') throw new Exception("Error : OAuth provider subject id required", 1);
    ...
    if(count($r) == 0){
      $legacyOauthUser = $this->_migrateLegacyOauthIdentity($oauth, $oauthId);
      if(!is_null($legacyOauthUser)) $r = [$legacyOauthUser];
    }

    if(count($r) > 0){
      return $this->_login($r[0]['email_user'], $oauthId, $oauth->_getOauthName(), null, true);
    } else {
  ...
  public function _createUser($email = null, $name = null, $password = null, $picture = "", $oauth = 'standard', $pushbullet = "", $twostep = 0, $admin = 0, $setpwd = false){
    ...
    $r = parent::execSqlRequest("INSERT INTO user_krypto (email_user, name_user, password_user, picture_user,
                                                          ...
                                                          [
                                                            'email_user' => $email,
                                                            'name_user' => $name,
                                                            'password_user' => self::_storedCredentialForNewUser($password, $oauth, $setpwd),
  ...
  private function _generateUserResetToken($Email){
    ...
    // Generate token
    $token = self::_generateResetPasswordTokenValue();
    $createdAt = (string) time();

    // Save new token in SQL
    $r = parent::execSqlRequest("UPDATE user_krypto SET reset_token_user=:reset_token_user, reset_token_created_user=:reset_token_created_user WHERE email_user=:email_user AND oauth_user=:oauth_user",
                                [
                                  'email_user' => $Email,
                                  'reset_token_user' => $token,
                                  'reset_token_created_user' => $createdAt,
                                  'oauth_user' => 'standard'
                                ]);
  ...
  /**
   * Invalidate password reset tokens for a standard user.
   * @param  String      $Email User email
   * @param  String|null $token Optional token value to invalidate atomically
   * @return Boolean
   */
  private function _invalidateResetTokensForEmail($Email, $token = null){
    $query = "UPDATE user_krypto SET reset_token_user = NULL, reset_token_created_user = NULL WHERE email_user=:email_user AND oauth_user=:oauth_user";
    $params = [
      'email_user' => $Email,
      'oauth_user' => 'standard'
    ];
    ...
    if(!is_null($token)){
      $query .= " AND reset_token_user=:reset_token_user";
      $params['reset_token_user'] = $token;
    }

    $r = parent::execSqlRequest($query, $params);
    if(!$r) throw new Exception("Error : Fail to invalidate reset token", 1);
  ...
  public function _parseToken($App = null, $token = null, $deleteToken = false){
    ...
    // Check token validity
    if(count($tokenDecode) != 2 || !filter_var($tokenDecode[0], FILTER_VALIDATE_EMAIL) || !self::_isValidResetPasswordToken($tokenDecode[1])) return false;
    ...
    if(self::_isResetPasswordTokenExpired($tokenMatch[0]['reset_token_created_user'])){
      $this->_invalidateResetTokensForEmail($tokenDecode[0], $tokenDecode[1]);
      return false;
    }
  ...
  public function _validResetPassword($token, $App, $password){
    ...
    // Check token given
    $tokenParsed = $this->_parseToken($App, $token);
    if(!$tokenParsed) throw new Exception("Error : Fail to parse token", 1);

    // Update new password
    $u = parent::execSqlRequest("UPDATE user_krypto SET password_user=:password_user, reset_token_user = NULL, reset_token_created_user = NULL WHERE email_user=:email_user AND reset_token_user=:reset_token_user AND oauth_user=:oauth_user",
                                [
                                  'email_user' => $tokenParsed[0],
                                  'reset_token_user' => $tokenParsed[1],
                                  'oauth_user' => 'standard',
  ...
  public function _saveChange($reloadsession = true){
    ...
    if($this->resetTokensNeedInvalidation){
      $this->_invalidateResetTokensForEmail($this->_getEmail());
      $this->resetTokensNeedInvalidation = false;
    }
